...
export default class AgendaView extends Component {
  ...
  initialScrollPadPosition() {
    return Math.max(0, this.viewHeight - this.props.headerHeight);
  }
  ...
  render() {
    ...
    const weekdaysStyle = [
      this.styles.weekdays,
      {
        opacity: this.state.scrollY.interpolate({
          inputRange: [agendaHeight - this.props.headerHeight, agendaHeight],
          outputRange: [0, 1],
          extrapolate: 'clamp'
        }),
        transform: [
          {
            translateY: this.state.scrollY.interpolate({
              inputRange: [
                Math.max(0, agendaHeight - this.props.headerHeight),
                agendaHeight
              ],
    ...
    if (!this.state.calendarIsReady) {
      ...
      weekdaysStyle.push({height: this.props.headerHeight});
    }

    const shouldAllowDragging =
      !this.props.hideKnob && !this.state.calendarScrollable;
    const scrollPadPosition =
      (shouldAllowDragging ? this.props.headerHeight : 0) -
      this.props.knobHeight;

    const scrollPadStyle = {
      position: 'absolute',
      width: 80,
      height: this.props.knobHeight,
    ...
    return (
      <View
        ...
        <View style={[this.styles.reservations, {marginTop: this.props.headerHeight}]}>
        ...
        <Animated.ScrollView
          ...
          <View
            ...
            style={{height: agendaHeight + this.props.knobHeight}}
